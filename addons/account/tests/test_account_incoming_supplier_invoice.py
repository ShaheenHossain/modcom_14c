...
class TestAccountIncomingSupplierInvoice(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.env['ir.config_parameter'].sudo().set_param('mail.catchall.domain', 'test-company.modcom.ca')

        cls.internal_user = cls.env['res.users'].create({
            'name': 'Internal User',
            'login': 'internal.user@test.modcom.ca',
            'email': 'internal.user@test.modcom.ca',
        })

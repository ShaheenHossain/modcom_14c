...
class AcquirerOdooByAdyen(models.Model):
    ...
    provider = fields.Selection(selection_add=[
       ('odoo_adyen', 'of ModCom Payments by Adyen')
    ...
    def _check_odoo_adyen_test(self):
        for payment_acquirer in self:
            if payment_acquirer.provider == 'odoo_adyen' and payment_acquirer.state == 'test':
                raise ValidationError(_('of ModCom Payments by Adyen is not available in test mode.'))
...
class TxOdooByAdyen(models.Model):
    ...
    def _odoo_adyen_form_get_tx_from_data(self, data):
        ...
        if not reference:
            error_msg = _('ModCom Payments by Adyen: received data with missing reference (%s)', reference)
            _logger.info(error_msg)
            raise ValidationError(error_msg)

        tx = self.env['payment.transaction'].search([('reference', '=', reference)])
        if not tx or len(tx) > 1:
            error_msg = _('ModCom Payments by Adyen: received data for reference %s') % (reference)
            if not tx:
    ...
    def _odoo_adyen_form_validate(self, data):
        ...
        if data['success']:
            ...
            error = _('ModCom Payment by Adyen: feedback error')
            _logger.info(error)

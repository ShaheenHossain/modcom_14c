...
class TestOdoobot(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.odoobot_ping_body = '<a href="http://modcom.ca/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@OdooBot</a>' % (cls.odoobot.id, cls.odoobot.id)
    ...
    def test_odoobot_ping(self):
        ...
        # ModCombot should not be a follower but user_employee and user_admin should
